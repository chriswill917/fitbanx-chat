...
import db, { auth, createTimestamp, messaging } from './firebase';
...
function App() {
  ...
  let clientIds = [];
  if(loginData.clientIds) {
    clientIds = loginData.clientIds.split(',')
  }

  
  const [users, fetchUsers] = useFetchData(30, db.collection("users").orderBy("timestamp", "desc"), true, clientIds, snap => {
      const data = [];
      ...
      {!user && !loader && !checkingVersion && !updating ?
        ...
        : user && !updating && chatsFetched ?
          <div className="app__body">
            <Sidebar chats={chats} pwa={pwaEvent} users={users} fetchUsers={fetchUsers} />
