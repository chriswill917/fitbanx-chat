...
export default memo(function ChatFooter({
	...
	const dbChange = useCallback(debounce((e) => {
        change(e)
    }, 300), []);
		...
		<div className="chat__footer" onBlur={handleBlur}>
			<form>
				<input
					ref={inputRef}
					onClick={handleFocus}
					onChange={!recording ? handleChange : null}
					onKeyPress={(event) => {
						if(event.key === 'Enter'){
				...
				{/* {navigator.mediaDevices.getUserMedia && window.MediaRecorder ? (
					...
					<>
						<label for="capture" class="send__btn">
							{btnIcons}
						</label>
						<input
							style={{ display: 'none' }}
