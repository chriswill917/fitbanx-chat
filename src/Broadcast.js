...
function Broadcast({ animState, isDesktop, unreadMessages, b, photos }) {
    const [input, setInput] = useState('');
    const { broadcastID, broadcastID: roomID } = useParams();
  ...
  //   //window
    ...
    // const addMessageToBroadcastRoom = async () => {
    //     const doc = await db.collection("rooms").doc(room.roomID).collection("messages").add(messageToSend);
    //     if (imageToUpload) {
    ...
    //             db.collection("rooms").doc(room.roomID).collection("messages").doc(doc.id).update({
    //                 imageUrl: url
    //             });
    //             if (room.userID) db.collection("notifications").add({
    //                 userID: room.userID,
    //                 title: room.displayName || room.name,
    //                 body: inputText,
    //                 photoURL: room.photoURL ? room.photoURL : null,
    //                 toUserId: room.userID,
    //                 image: url
    //             });
    ...
    const sendMessage = async event => {
        ...
        if (input !== "" || (input === "" && (image || video))) {
            ...
            const roomIDs = location.state.broadcastUsers;
            const broadcastRoomID = location.state?.roomID;


            if (Array.isArray(roomIDs) && roomIDs?.length) {
                if (broadcastRoomID) {
                    const recipients = roomIDs.map(item => {
                        const updatedItem = { ...item };
                        delete updatedItem.avtarStyle;
                        return updatedItem;
                    })
                    db.collection("broadcast").doc(broadcastRoomID).set({
                        ...roomInfo,
                        isBroadcast: true,
                ...
                // add message to broadcast room
                const broadcastMessageDoc = await db.collection("broadcast").doc(broadcastRoomID).collection("messages").add(messageToSend);

                roomIDs.forEach(async (room) => {
                    db.collection("rooms").doc(room.roomID).set(roomInfo, { merge: true });
                    if (room.userID) {
                        db.collection("users").doc(room.userID).collection("chats").doc(room.roomID).set({
                            timestamp: createTimestamp(),
                        ...
                        db.collection("users").doc(user.uid).collection("chats").doc(room.roomID).set({
                            timestamp: createTimestamp(),
                            photoURL: room.photoURL ? room.photoURL : null,
                            name: room.name,
                            userID: room.userID
                    ...
                    if (imageToUpload) {
                        if (imageToUpload.isCaptureImage) {
                            ...
                            db.collection("broadcast").doc(broadcastRoomID).collection("messages").doc(broadcastMessageDoc.id).update({
                                imageUrl: url
                            });
                            if (room.userID) db.collection("notifications").add({
                                userID: room.userID,
                    ...
                    if (!isNoticationSend && room.userID) {
	...
	if (window.navigator.onLine) {
            ...
            setDeleting(true);
            try {
                const room = db.collection("broadcast").doc(roomID);
    ...
    function fetchMessages(update) {
        const ref = db.collection("broadcast").doc(roomID).collection("messages").orderBy("timestamp", "desc");
        if (update) {
    ...
    function listenToImg(docID) {
        return db.collection("broadcast").doc(roomID).collection("messages").doc(docID).onSnapshot(doc => {
    ...
    function listenToAudio(docID) {
        ...
        const s = db.collection("broadcast").doc(roomID).collection("messages").doc(docID).onSnapshot(doc => {
    ...
    useEffect(() => {
        if (roomID && !deleting) {
            ...
            if (dispatchMessages[roomID]?.firstMessageID) {
                ...
                db.collection("broadcast").doc(roomID).collection("messages").orderBy("timestamp", "asc").limit(1).get().then(docs => {
  ...
  //   //window
    ...
    useEffect(() => {
        if (roomID && state?.userID) {
            console.log(roomID);
            var h = db.collection("broadcast").doc(roomID).onSnapshot(snap => {
                if (snap.data()) {
    ...
    useEffect(() => {
        if (roomID) {
            var z = db.collection("broadcast").doc(roomID).onSnapshot(snap => {
        ...
        var a = () => {
            ...
            if (messages.length > 0) {
                if (chatBodyContainer.current.scrollTop > chatBodyContainer.current.scrollHeight - chatBodyContainer.current.offsetHeight - 180
                    ...
                    db.collection("broadcast").doc(roomID).set({
                        seen: true,
                    }, { merge: true });
                }
                if (chatBodyContainer.current.scrollTop > chatBodyContainer.current.scrollHeight - chatBodyContainer.current.offsetHeight - 180
  ...
  //   }
    ...
    return (
        ...
        <div ref={chatAnim} className="chat">
            <div style={{
                height: page.height,
            ...
            <div className="chat__header">
                ...
                >
                    ...
                    <Menu
                        ...
                        keepMounted
                    >
                        <MenuItem onClick={deleteRoom}>Delete Broadcast</MenuItem>
                    </Menu>
                </div>
            </div>
